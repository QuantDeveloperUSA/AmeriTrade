...
using AmeriTrade.API.Extensions;
...
namespace AmeriTrade.API
{
    ...
    public class AmeritradeClient : IDisposable
    {
        ...
        public async Task<bool> LogIn( Autentication autentication)
        {
            if (string.IsNullOrWhiteSpace(autentication.userName))
            {
                //throw new ArgumentException(string.Format(Errors.CannotBeNullOrWhitespace, "userName"), "userName");
            }

            if (string.IsNullOrWhiteSpace(autentication.password))
            {
            ...
            var response = await this.http.PostAsync(
                "/apps/300/LogIn?source=" + Uri.EscapeDataString(autentication.key) + "&version=" + Uri.EscapeDataString(this.version),
                new FormUrlEncodedContent(new[]
                {
                    new KeyValuePair<string, string>("userid", autentication.userName),
                    new KeyValuePair<string, string>("password", autentication.password),
                    new KeyValuePair<string, string>("source", autentication.key),
                    new KeyValuePair<string, string>("version", this.version)
                }));

            autentication.userName = autentication.password = null;
        ...
        {
            var text = await this.http.GetStringAsync("/apps/100/LogOut?source=" + Uri.EscapeDataString(key));
            var xml = XDocument.Parse(text);
        ...
        public async Task KeepAlive(string key)
        {
            this.EnsureAuthenticated();

            var text = await this.http.GetStringAsync("/apps/KeepAlive?source=" + Uri.EscapeDataString(key));
        ...
        public async Task GetStreamerInfo(string key,string accountID = null)
        {
            this.EnsureAuthenticated();

            var url = "/apps/100/StreamerInfo?source=" + Uri.EscapeDataString(key) +
        ...
        public async Task<List<object>> GetQuotes(string key,params string[] symbols)
        {
            if (symbols == null)
            {
            ...
            var url = "/apps/100/Quote?source=" + Uri.EscapeDataString(key) +
        ...
        public async Task<Dictionary<string, Quote[]>> GetHistoricalPrices(string key,string[] symbols, DateTime? startDate = null, DateTime? endDate = null)
        {
            if (symbols == null)
            {
                throw new ArgumentNullException("symbols");
            ...
            var url = "/apps/100/PriceHistory?source=" + Uri.EscapeDataString(key) + "&requestidentifiertype=SYMBOL" +
        ...
        public async Task<Dictionary<string, string>> FindSymbols(string key,string search)
        {
            ...
            var text = await this.http.GetStringAsync(
                "/apps/100/SymbolLookup?source=" + Uri.EscapeDataString(key) +
        ...
        public async Task<List<Watchlist>> GetWatchlists(string key)
        {
            this.EnsureAuthenticated();

            var xml = await this.http.GetXmlAsync("/apps/100/GetWatchlists?source=" + Uri.EscapeDataString(key));
